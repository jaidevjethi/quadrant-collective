...
5. **Local-SEO layer (next phase — do it right, not as doorway pages):** confirmed strategy — one **registered base city** (rank hardest there: real GBP + `LocalBusiness` schema + one genuine location page); the other two corridor cities via service-area + content, NOT fake location pages. Then: deep per-service pages (`/services/[service]`) as the primary keyword assets; a small set of service-in-base-city pages with unique content, expanded one at a time; `/insights` blog for topical authority. Technical: `sitemap.xml`, `robots.txt`, OG images per route, `Service`/`FAQ` schema. Then analytics (GA + Clarity, `lazyOnload`).
