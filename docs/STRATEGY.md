...
4. ✅ Multi-page core (2026-07-04): sticky `SiteHeader` nav, `/contact` route (shares `ContactForm` with the homepage beat), `/about` (honest solo-studio copy from brand facts — founder name/photo/bio deliberately left as founder-supplied placeholders), `/work` index + `/work/[slug]` template (empty-ready; ships with real projects only). Per-route metadata + `metadataBase` (`src/lib/site.ts` `SITE_URL`, provisional) + canonicals. All routes static/SSG, verified via production build. Needs from founder: registered base city, About personal content, real case studies.
5. **Local-SEO layer (next phase — do it right, not as doorway pages):** confirmed strategy — one **registered base city** (rank hardest there: real GBP + `LocalBusiness` schema + one genuine location page); the other two corridor cities via service-area + content, NOT fake location pages. Then: a small set of service-in-base-city pages with unique content, expanded one at a time; `/insights` blog for topical authority. Then analytics (GA + Clarity, `lazyOnload`).
6. ✅ Ship phase (2026-07-06): **live on GitHub Pages** (`jaidevjethi/quadrant-collective`, Actions workflow, static export with `NEXT_PUBLIC_BASE_PATH`; custom domain swaps in via CNAME later). **Contact is WhatsApp-first (founder decision):** the form composes a formatted message to +91 97734 56668 (`src/lib/whatsapp.ts`), no server; brand-styled floating WhatsApp button site-wide; `tel:` on Contact. Supersedes the old "wire /api/contact to email/CRM" follow-up. `/services` page live with the founder's real packages (social media, Google Business Profile, website & SEO; healthcare/local-practice framing). About founder section is real: Jaidev Jethi, portrait, bio (Ganpat University → Canada → Microsoft). SEO: `sitemap.ts`, `robots.ts`, static OG card (`public/og.png`), Organization+founder/Person/Service schema. Motion: hero load choreography, blueprint-entry moment, route fade, invitation flow streak. Responsive: audited 360–1440 on all routes, zero horizontal overflow.
